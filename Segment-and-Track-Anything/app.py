...
def get_meta_from_img_seq(input_img_seq):
    ...
    file_path = os.path.abspath(f'./assets/{file_name}')
    ...
    os.system(f'unzip -q "{input_img_seq.name}" -d "{file_path}"')
...
def seg_track_app():
    ...
    ##########################################################
        ...
        video_example = os.path.join(os.path.dirname(__file__), "assets", "blackswan.mp4")
        if os.path.exists(video_example):
        ...
        with gr.Tab(label='Image-seq expamle'):
            gr.Examples(
                examples=[
                    os.path.join(os.path.dirname(__file__), "assets", "images.zip"),
                ],
