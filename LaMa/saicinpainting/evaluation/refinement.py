...
def _infer(
    ...
    scale_ind : int, n_iters : int=15, lr : float=0.002):   # (image, mask, forward_front, forward_rears, image_inpainted, orig_shape, devices, ids, n_iters, lr)
    ...
    pbar = tqdm(range(n_iters), leave=False)   # 15 iterations
    for idi in pbar:
        optimizer.zero_grad()
        input_feat = (z1,z2)
...
def _get_image_mask_pyramid(batch : dict, min_side : int, max_scales : int, px_budget : int):
    ...
    if h*w > px_budget:
        ...
        # print(f"The current resize ratio is {ratio}.")
        h_orig, w_orig = h, w
...
def refine_predict(
    ...
    # print(f"The current GPU is {gpu_ids}.")
