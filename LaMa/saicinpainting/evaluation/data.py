...
def load_image(datadir, fname, mode='RGB', return_orig=False):

    if mode == "npy":  
        depth_original_path = os.path.join(datadir, "depth_original", fname.split('/')[-1])
        ...
        depth = depth * 255.0
        img = np.stack([depth, depth, depth], -1) 
    else:
...
class InpaintingDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if "npy" in self.img_suffix: 
            image = load_image(self.datadir, self.img_filenames[i], mode='npy')   # for depth inpainting
        else:
            image = load_image(self.datadir, self.img_filenames[i], mode='RGB') 
        
        mask = load_image(self.datadir, self.mask_filenames[i], mode='L') 
